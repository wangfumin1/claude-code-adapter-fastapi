...
from .services import MessageConverter, OpenAIClient, ToolSelectionClient, ResponseProcessor
    ...
    try:
        ...
        if settings.enable_tool_selection and tools:
            if tool_choice:
                ...
                selected_tools = await select_tools(body.get("model"), recent_msgs, tools)
        ...
        # 记录实际调用目标
        logger.info(f"请求地址：{openai_client.client.base_url}，"
        ...
        if stream_mode:
            async def event_stream():
                try:
                    ...
                    error_data = {
                        "type": "error",
                        "error": {"type": "api_error", "message": f"request failed: {str(e)}"}
                    }
            ...
            # 处理响应
            anthropic_resp = response_processor.process_response(lm_resp, settings.target_model_config['model'])
...
async def select_tools(target_model: str, recent_msgs: List[Dict[str, Any]], all_tools: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    if not all_tools:
        return []
    
    # 构建待选择工具列表
    tools_list = "\n".join(f"{{{t['name']}: '{t['description'][0:100]}...'}}," for t in all_tools)
    
    # 构建最近消息列表
    recent_str = "\n\n".join(f"{{{m['role']}: '{flatten_content(m['content'])}'}}," for m in recent_msgs)
