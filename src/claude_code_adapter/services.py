...
class MessageConverter:
    ...
    def convert_anthropic_to_openai_messages(self, body: Dict[str, Any]) -> List[Dict[str, Any]]:
...
class ResponseProcessor:
    ...
    def process_response(self, lm_resp: Dict[str, Any], target_model: str) -> Dict[str, Any]:
        ...
        for choice in lm_resp.get("choices", []):
            ...
            if tool_calls:
                ...
                # 先添加可能存在的文本内容
                text_content = re.sub(r'```json.*?```', '', content, flags=re.DOTALL).strip()
                if text_content:
        ...
        logger.info(f"返回响应: {len(content_blocks)} 个块, stop_reason={anthropic_resp['stop_reason']}")
        logger.debug(f"响应内容: {json.dumps(anthropic_resp, ensure_ascii=False, indent=2)}")
